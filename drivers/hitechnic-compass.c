...
bool HTMCstartCal(sensor_port_t *link) {
  ...
  return writeI2C(link, HTMC_I2CRequest, NULL, 0);
}
...
int HTMCreadRelativeHeading(sensor_port_t *link) {
  ...
  int _tmpHeading = HTMCreadHeading(link) - target[link->portNum][0] + 180;
...
int HTMCsetTarget(sensor_port_t *link, int offset) {
  target[link->portNum][0] = (offset != 0) ? offset : HTMCreadHeading(link);
  return target[link->portNum][0];
}
